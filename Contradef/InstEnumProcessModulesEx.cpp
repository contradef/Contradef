...
std::map<CallContextKey, CallContext*> InstEnumProcessModulesEx::callContextMap;
UINT32 InstEnumProcessModulesEx::imgCallId = 0;
UINT32 InstEnumProcessModulesEx::fcnCallId = 0;
Notifier* InstEnumProcessModulesEx::globalNotifierPtr;


VOID InstEnumProcessModulesEx::CallbackBefore(THREADID tid, UINT32 callId, ADDRINT instAddress, ADDRINT rtn, CONTEXT* ctx, ADDRINT returnAddress,
...
VOID InstEnumProcessModulesEx::CallbackAfter(THREADID tid, UINT32 callId, ADDRINT instAddress, ADDRINT rtn, CONTEXT* ctx, ADDRINT* retValAddr, ADDRINT returnAddress,
    ADDRINT hProcess, ADDRINT lphModule, ADDRINT cb, ADDRINT lpcbNeeded, ADDRINT dwFilterFlag) {
    ...
    if (it != callContextMap.end()) {
        ...
        const InstEnumProcessModulesExArgs* args = reinterpret_cast<InstEnumProcessModulesExArgs*>(callContext->functionArgs);
        ...
        stringStream << "        dwFilterFlag: " << args->dwFilterFlag << std::endl;
...
VOID InstEnumProcessModulesEx::InstrumentFunction(RTN rtn, Notifier& globalNotifier) {
    std::string rtnName = RTN_Name(rtn);
    if (rtnName == "EnumProcessModulesEx") {
        imgCallId++;
        ...
        RTN_InsertCall(rtn, IPOINT_BEFORE, (AFUNPTR)CallbackBefore,
            ...
            IARG_FUNCARG_ENTRYPOINT_VALUE, 4, // dwFilterFlag
            IARG_END);
