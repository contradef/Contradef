...
// Helper function to get the spritnf format
template<class T> inline const char* get_format() {
	puts("to_string used on unsupported type");
	abort();
	return ""; // Exists after abort
}
template<> inline const char* get_format<int>() { return "%d";}
template<> inline const char* get_format<long>() { return "%ld";}
template<> inline const char* get_format<long long>() { return "%lld";}
template<> inline const char* get_format<unsigned>() { return "%u";}
template<> inline const char* get_format<unsigned long>() { return "%lu";}
template<> inline const char* get_format<unsigned long long>() { return "%llu";}
template<> inline const char* get_format<float>() { return "%f";}
template<> inline const char* get_format<double>() { return "%lf";}
template<> inline const char* get_format<long double>() { return "%Lf";}

// Convert numerical value to string
// Returns a string with the representation of val.
template<class T>
string _STLP_CALL to_string(T value) {
  // Get the necessary number of required bytes to write (including '\0')
  int size = snprintf(NULL, 0, get_format<T>(), value) + 1; 
  if (size <= 0) 
  {
	  puts("to_string failed");
	  abort();
  }
  char *buf = new char[size];
  snprintf(buf, size, get_format<T>(), value);
...
//to_wstring
// Helper function to get the spritnf format
template<class T> inline const wchar_t* wget_format() {
	puts("to_string used on unsupported type");
	abort();
	return L""; // Exists after abort
}
template<> inline const wchar_t* wget_format<int>() { return L"%d";}
template<> inline const wchar_t* wget_format<long>() { return L"%ld";}
template<> inline const wchar_t* wget_format<long long>() { return L"%lld";}
template<> inline const wchar_t* wget_format<unsigned>() { return L"%u";}
template<> inline const wchar_t* wget_format<unsigned long>() { return L"%lu";}
template<> inline const wchar_t* wget_format<unsigned long long>() { return L"%llu";}
template<> inline const wchar_t* wget_format<float>() { return L"%f";}
template<> inline const wchar_t* wget_format<double>() { return L"%lf";}
template<> inline const wchar_t* wget_format<long double>() { return L"%Lf";}

// Convert numerical value to wide string
// Returns a wstring with the representation of val.
template<class T>
wstring _STLP_CALL to_wstring(T value) {
  // Get the necessary number of required bytes to write (including '\0')
  int size = snprintf(NULL, 0, get_format<T>(), value) + 1; 
  if (size <= 0) {
	  puts("to_string failed");
  ...
  swprintf(buf, size, wget_format<T>(), value);
...
// 
// If pos is not a null pointer, the function also sets the value of pos to the position of the first character in str after the number.
//
// Functions should be inlined because the source file is included from the header file
// what causes a compilation error due to multiple definitions
#  if defined (_STLP_HAS_WCHAR_T)
inline unsigned long long _STLP_CALL stoull(const wstring& str, size_t* pos = NULL , int base = 10 ) {
  wchar_t *ptr = NULL;
  const wchar_t *c_str = str.c_str();
  unsigned long long res;
  if(pos) {
	  res = wcstoull(c_str, &ptr, base);
	  *pos = ptr - c_str;
  }
  else res = wcstoull(c_str, NULL, base);
  return res;
}
#endif
inline unsigned long long _STLP_CALL stoull(const string& str, size_t* pos = NULL , int base = 10) {
  char *ptr = NULL;
  const char *c_str = str.c_str();
  unsigned long long res;
  if (pos) {
	  res = strtoull(c_str, &ptr, base);
	  *pos = ptr - c_str;
  }
  else res = strtoull(c_str, NULL, base);
  return res;
}
...
// what causes a compilation error due to multiple definitions
#  if defined (_STLP_HAS_WCHAR_T)
inline unsigned long _STLP_CALL stoul(const wstring& str, size_t* pos = NULL , int base = 10 ) {
  wchar_t *ptr = NULL;
  const wchar_t *c_str = str.c_str();
  unsigned long res;
  if(pos) {
	  res = wcstoul(c_str, &ptr, base);
	  *pos = ptr - c_str;
  }
  else res = wcstoul(c_str, NULL, base);
  return res;
}
#endif
inline unsigned long _STLP_CALL stoul(const string& str, size_t* pos = NULL , int base = 10) {
  char *ptr = NULL;
  const char *c_str = str.c_str();
  unsigned long res;
  if (pos) {
	  res = strtoul(c_str, &ptr, base);
	  *pos = ptr - c_str;
  }
  else res = strtoul(c_str, NULL, base);
  return res;
}
